...
from rcnn_dff.dataset import *
from rcnn_dff.core.loader import TestLoader
from rcnn_dff.core.tester import Predictor, pred_eval

from rcnn_dff.utils.load_model import load_param

def test_rcnn_dff(network, dataset, image_set, root_path, dataset_path,
              ctx, prefix, epoch,
    ...
    max_data_shape = [('data', (1, 3, max([v[0] + 8 for v in config.SCALES]), max([v[1] + 38 for v in config.SCALES])))]
    if not has_rpn:
...
def main():
    ...
    ctx = mx.gpu(args.gpu)
    print args
    test_rcnn_dff(args.network, args.dataset, args.image_set, args.root_path, args.dataset_path,
