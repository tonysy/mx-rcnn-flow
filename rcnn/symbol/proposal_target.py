...
class ProposalTargetOperator(mx.operator.CustomOp):
    def __init__(self, num_classes, batch_images, batch_rois, fg_fraction, ohem):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if self._ohem:
            rois, labels, bbox_targets, bbox_weights = \
                get_rois(all_rois, rois_per_image, self._num_classes, gt_boxes=gt_boxes)
        else:
...
class ProposalTargetProp(mx.operator.CustomOpProp):
    def __init__(self, num_classes, batch_images, batch_rois, fg_fraction='0.25', ohem='False'):
        super(ProposalTargetProp, self).__init__(need_top_grad=False)
        self._num_classes = int(num_classes)
        self._batch_images = int(batch_images)
        self._batch_rois = int(batch_rois)
        self._fg_fraction = float(fg_fraction)
        self._ohem = strtobool(ohem)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return ProposalTargetOperator(self._num_classes, self._batch_images, self._batch_rois, self._fg_fraction, self._ohem)
