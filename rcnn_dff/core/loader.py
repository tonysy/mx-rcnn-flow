...
from ..io.rpn import get_rpn_testbatch, get_rpn_testbatch_ffa, get_rpn_batch, get_rpn_batch_ffa, assign_anchor
from ..io.rcnn import get_rcnn_testbatch, get_rcnn_testbatch_ffa, get_rcnn_batch, get_rcnn_batch_ffa
...
class AnchorLoaderFFA(mx.io.DataIter):
    def __init__(self, feat_sym, roidb, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
                 feat_stride=16, anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2), allowed_border=0,
                 aspect_grouping=False):
        """
    ...
    def get_batch(self):
        ...
        for islice in slices:
            iroidb = [roidb[i] for i in range(islice.start, islice.stop)]
            data, label = get_rpn_batch_ffa(iroidb)
            data_list.append(data)
            label_list.append(label)
        # print "============data_list", data_list[0]['data'].shape
        # pad data first and then assign anchor (read label)
        ...
        # print "=====",data['data'].shape
        for i in range(config.FRAMES_FEATURE_AGGREGATION):
            data_temp_tensor = tensor_vstack([batch['prev_{}'.format(i+1)] for batch in data_list])
            for data, data_pad in zip(data_list, data_temp_tensor):
                data['prev_{}'.format(i+1)] = data_pad[np.newaxis, :]
            data_temp_tensor = tensor_vstack([batch['next_{}'.format(i+1)] for batch in data_list])
            for data, data_pad in zip(data_list, data_temp_tensor):
                data['next_{}'.format(i+1)] = data_pad[np.newaxis, :]
