...
def get_twin_path_ffa(roi_rec, rand_map, img_map, frame_index):
    """
    get twin pic of the training image
    only for kitti datanow!!!!
    """
    ...
    curr = ref + int(frame_index)
    img_info[2] = str(curr).zfill(10)
...
def get_image_ffa(roidb, crop='origin'):
    """
    preprocess image and return processed roidb
    :param roidb: a list of roidb
    :param crop: ['origin', 'center', 'random']
    ...
    if config.FRAMES_FEATURE_AGGREGATION > 0:
        ...
        for j in range(config.FRAMES_FEATURE_AGGREGATION):
            processed_ims_nearby['next_{}'.format(j+1)] = []
        ...
        if crop == 'random':
            ...
            if config.FRAMES_FEATURE_AGGREGATION > 0:
                ...
                for m in range(config.FRAMES_FEATURE_AGGREGATION):
                    ...
                    if not os.path.exists(twin_path):
                        processed_ims_nearby['prev_{}'.format(m+1)].append(im_tensor)
                        print "twin pic {} does not exist\n".format(twin_path)
                    else:
                ...
                for m in range(config.FRAMES_FEATURE_AGGREGATION):
                    twin_path = get_twin_path_ffa(roi_rec, rand_map, img_map,frame_index=(m+1))
                    if not os.path.exists(twin_path):
                        processed_ims_nearby['next_{}'.format(m+1)].append(im_tensor)
                        print "twin pic {} does not exist\n".format(twin_path)
                    else:
                        im_temp = cv2.imread(twin_path)
                        if roidb[i]['flipped']:
