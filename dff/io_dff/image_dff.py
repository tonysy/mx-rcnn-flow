...
def get_image_(roidb, crop='origin'):
    """
...
def get_image(roidb, crop='origin'):
    ...
    rand_map = []
    img_map = []
    with open(dff_config.TRAIN.RAND_FILE) as f:
        for line in f:
            rand_map = line.strip().split(',')
    with open(dff_config.TRAIN.MAP_FILE) as f:
        for line in f:
    ...
    for i in range(num_images):
        ...
        if crop == 'random':
            ...
            # ====== modified for deep feature flow
            new_rec = roi_rec.copy()
            scale_ind = random.randrange(len(config.SCALES))
            target_size = config.SCALES[scale_ind][0]
            max_size = config.SCALES[scale_ind][1]
            ...
            # ====== next frame image path
            twin_path = next_image_path(roi_rec, rand_map, img_map)
            if not os.path.exists(twin_path):
                processed_im2.append(im_tensor)
                print "Twin pic {} does not exists\n".format(twin_path)
            else:
                im2 = cv2.imread(twin_path)
                if roidb[i]['flipped']:
                    im2 = im2[:, ::-1,:]
                im2, _ = resize(im2, target_size, max_size, stride=dff_config.IMAGE_STRIDE)
                im2_tensor = transform(im2, dff_config.PIXEL_MEANS)
...
def next_image_path(roi_rec, rand_map, img_map):
    ...
    img_path = os.path.join(dff_config.TRAIN.RAW_DATA, img_info[0], \
                            img_info[1],'image_02', 'data', \
                            img_info[2]+'.png')
